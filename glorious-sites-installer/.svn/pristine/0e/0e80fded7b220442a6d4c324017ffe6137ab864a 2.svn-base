 ...
 *
 * @package GloriousThemes_Demo_Importer\Functions
 * @version 1.0.0
 */
...
function tg_ajax_install_required_plugin() {
	...
	if ( empty( $_POST['plugin'] ) || empty( $_POST['slug'] ) ) {
		wp_send_json_error(
			array(
				...
				'errorMessage' => __( 'No plugin specified.', 'glorious-sites-installer' ),
			)
		);
	}
	...
	if ( ! current_user_can( 'install_plugins' ) ) {
		$status['errorMessage'] = __( 'Sorry, you are not allowed to install plugins on this site.', 'glorious-sites-installer' );
 ...
 */
if ( apply_filters( 'gloriousthemes_clear_data_before_demo_import', true ) ) {
	add_action( 'gloriousthemes_ajax_before_demo_import', 'tg_reset_widgets', 10 );
	add_action( 'gloriousthemes_ajax_before_demo_import', 'tg_delete_nav_menus', 20 );
	add_action( 'gloriousthemes_ajax_before_demo_import', 'tg_remove_theme_mods', 30 );
}
 ...
 */
add_action( 'gloriousthemes_ajax_demo_imported', 'tg_set_elementor_load_fa4_shim' );
 ...
 */
add_action( 'gloriousthemes_ajax_demo_imported', 'tg_set_elementor_active_kit' );
 ...
 * @see tg_set_wc_pages()
 */
add_action( 'gloriousthemes_ajax_demo_imported', 'tg_set_wc_pages' );
...
function tg_set_wc_pages( $demo_id ) {
	if ( class_exists( 'WooCommerce' ) ) {
		...
		$wc_pages = apply_filters(
			'gloriousthemes_wc_' . $demo_id . '_pages',
			array(
				'shop'      => array(
					'name'  => 'shop',
					'title' => 'Shop',
				...
				<# if ( data.successes ) { #>
					<# if ( 1 === data.successes ) { #>
						<# if ( 'plugin' === data.type ) { #>
							...
							printf( __( '%s plugin successfully installed.', 'glorious-sites-installer' ), '{{ data.successes }}' );
							?>
						<# } #>
					<# } else { #>
						<# if ( 'plugin' === data.type ) { #>
							<?php
							/* translators: %s: Number of plugins */
							printf( __( '%s plugins successfully installed.', 'glorious-sites-installer' ), '{{ data.successes }}' );
							?>
						<# } #>
					<# } #>
				<# } #>
				<# if ( data.errors ) { #>
					<button class="button-link bulk-action-errors-collapsed" aria-expanded="false">
						<# if ( 1 === data.errors ) { #>
							...
							printf( __( '%s install failed.', 'glorious-sites-installer' ), '{{ data.errors }}' );
							?>
						<# } else { #>
							<?php
							/* translators: %s: Number of failed installs */
							printf( __( '%s installs failed.', 'glorious-sites-installer' ), '{{ data.errors }}' );
							?>
						<# } #>
						<span class="screen-reader-text"><?php _e( 'Show more details', 'glorious-sites-installer' ); ?></span>
