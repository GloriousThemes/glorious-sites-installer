<?php
/**
 * GloriousThemes Starter Sites.
 *
 * @package GloriousThemes_Demo_Importer\Classes
 * @version 1.0.0
 */
...
class TG_Demo_Importer {
	...
	public function __construct() {
		...
		if ( apply_filters( 'gloriousthemes_show_demo_importer_page', true ) ) {
		...
		// Help Tabs.
		if ( apply_filters( 'gloriousthemes_demo_importer_enable_admin_help_tab', true ) ) {
		...
		add_action( 'gloriousthemes_ajax_demo_imported', array( $this, 'update_nav_menu_items' ) );
		add_action( 'gloriousthemes_ajax_demo_imported', array( $this, 'update_elementor_data' ), 10, 2 );
		add_action( 'gloriousthemes_ajax_demo_imported', array( $this, 'update_siteorigin_data' ), 10, 2 );

		// Update widget and customizer demo import settings data.
		add_filter( 'gloriousthemes_widget_demo_import_settings', array( $this, 'update_widget_data' ), 10, 4 );
		add_filter( 'gloriousthemes_customizer_demo_import_settings', array( $this, 'update_customizer_data' ), 10, 2 );
	...
	private function get_demo_packages() {
		$packages = get_transient( 'gloriousthemes_demo_importer_packages' );
		$template = strtolower( str_replace( '-pro', '', get_option( 'template' ) ) );

		if ( false === $packages || ( isset( $packages->slug ) && $template !== $packages->slug ) ) {
			//Github link removed as per WP Requirements
			$raw_packages = wp_safe_remote_get( "https://raw.githubusercontent.com/GloriousThemes/gloriousthemes-demo-content/master/configs/{$template}.json" );
			/*
			 * Glorious Themes OWN CDN Storage added
			 * URL - https://gloriousproducts.b-cdn.net/ (Bunny CDN link)
			 * 
			 * Note for the WordPress Team ( Reviewer )
			 * Thank you for helping us to improve this plugin, we are hosting the json files and configs to remote storage 
			 * because they will be updated every week.
			 * Since WordPress prohibits frequent commits to its repo, we are uploading the json and the sample data on our own CDN
			 * so that the plugin will receive important updates and the demo data can receive frequent commits.
			*
			//$raw_packages = wp_safe_remote_get( "https://gloriousproducts.b-cdn.net/GloriousThemes/gloriousthemes-demo-content/configs/{$template}.json" );

			$raw_packages = wp_safe_remote_get( plugin_dir_url ( __DIR__ ) . "/gloriousthemes-demo-content/configs/{$template}.json" ); 
			...
			*/
			...
			if ( ! is_wp_error( $raw_packages ) ) {
				...
				if ( $packages ) {
					set_transient( 'gloriousthemes_demo_importer_packages', $packages, WEEK_IN_SECONDS );
				}
			}
		}

		return apply_filters( 'gloriousthemes_demo_importer_packages_' . $template, $packages );
	}
	...
	public function admin_menu() {
		add_theme_page( __( 'Glorious Demo Importer', 'glorious-sites-installer' ), __( 'Glorious Demo Importer', 'glorious-sites-installer' ), 'switch_themes', 'demo-importer', array( $this, 'demo_importer' ) );
		add_theme_page( __( 'Demo Importer Status', 'glorious-sites-installer' ), __( 'Demo Importer Status', 'glorious-sites-installer' ), 'switch_themes', 'demo-importer-status', array( $this, 'status_menu' ) );
	}
	...
	public function add_menu_classes() {
		...
		if ( isset( $submenu['themes.php'] ) ) {
			...
			if ( apply_filters( 'gloriousthemes_demo_importer_include_class_in_menu', true ) ) {
				foreach ( $submenu['themes.php'] as $order => $menu_item ) {
					if ( 0 === strpos( $menu_item[0], _x( 'Demo Importer', 'Admin menu name', 'glorious-sites-installer' ) ) ) {
			...
			wp_localize_script(
				...
				array(
					...
					'settings' => array(
						...
						'suggestURI'    => apply_filters( 'gloriousthemes_demo_importer_suggest_new', 'https://gloriousthemes.com/contact/' ),
						'confirmImport' => sprintf(

							__( 'Importing demo data will ensure that your site will look similar as theme demo. It makes you easy to modify the content instead of creating them from scratch. Also, consider before importing the demo: %1$s %2$s %3$s %4$s %5$s %6$s', 'glorious-sites-installer' ),

							'<ol><li class="warning">' . __( 'Importing the demo on the site if you have already added the content is highly discouraged.', 'glorious-sites-installer' ) . '</li>',

							'<li>' . __( 'You need to import demo on fresh WordPress install to exactly replicate the theme demo.', 'glorious-sites-installer' ) . '</li>',

							'<li>' . __( 'It will install the required plugins as well as activate them for installing the required theme demo within your site.', 'glorious-sites-installer' ) . '</li>',

							'<li>' . __( 'Copyright images will get replaced with other placeholder images.', 'glorious-sites-installer' ) . '</li>',

							'<li>' . __( 'None of the posts, pages, attachments or any other data already existing in your site will be deleted or modified.', 'glorious-sites-installer' ) . '</li>',

							'<li>' . __( 'It will take some time to import the theme demo.', 'glorious-sites-installer' ) . '</li></ol>'

							)
					),
					'l10n'     => array(
						'search'              => __( 'Search Demos', 'glorious-sites-installer' ),
						'searchPlaceholder'   => __( 'Search demos...', 'glorious-sites-installer' ), // placeholder (no ellipsis)
						/* translators: %s: support forums URL */
						'error'               => sprintf( __( 'An unexpected error occurred. Something may be wrong with GloriousThemes demo server&#8217;s configuration. If you continue to have problems, please try the <a href="%s">support forums</a>.', 'glorious-sites-installer' ), 'https://wordpress.org/support/plugin/glorious-sites-installer' ),
						'tryAgain'            => __( 'Try Again', 'glorious-sites-installer' ),
						'suggestNew'          => __( 'Please suggest us!', 'glorious-sites-installer' ),
						'demosFound'          => __( 'Number of Demos found: %d', 'glorious-sites-installer' ),
						'noDemosFound'        => __( 'No demos found. Try a different search.', 'glorious-sites-installer' ),
						'collapseSidebar'     => __( 'Collapse Sidebar', 'glorious-sites-installer' ),
						'expandSidebar'       => __( 'Expand Sidebar', 'glorious-sites-installer' ),
						/* translators: accessibility text */
						'selectFeatureFilter' => __( 'Select one or more Demo features to filter by', 'glorious-sites-installer' ),
						'confirmMsg'          => __( 'Confirm!', 'glorious-sites-installer' ),
					),
				)
			);

			// For translation of strings within scripts.
			wp_set_script_translations( 'tg-demo-updates', 'glorious-sites-installer' );
		}
	}
	...
	public function admin_footer_text( $footer_text ) {
		...
		// Check to make sure we're on a GloriousThemes Starter Sites admin page.
		if ( isset( $current_screen->id ) && apply_filters( 'gloriousthemes_demo_importer_display_admin_footer_text', in_array( $current_screen->id, array( 'appearance_page_demo-importer' ) ) ) ) {
			// Change the footer text.
			if ( ! get_option( 'gloriousthemes_demo_importer_admin_footer_text_rated' ) ) {
				$footer_text = sprintf(
					/* translators: 1: GloriousThemes Starter Sites 2: five stars */
					__( 'If you like %1$s please leave us a %2$s rating. A huge thanks in advance!', 'glorious-sites-installer' ),
					sprintf( '<strong>%s</strong>', esc_html__( 'GloriousThemes Starter Sites', 'glorious-sites-installer' ) ),
					'<a href="https://wordpress.org/support/plugin//glorious-sites-installer/reviews?rate=5#new-post" target="_blank" class="glorious-sites-installer-rating-link" data-rated="' . esc_attr__( 'Thanks :)', 'glorious-sites-installer' ) . '">&#9733;&#9733;&#9733;&#9733;&#9733;</a>'
				);
			} else {
				$footer_text = __( 'Thank you for importing with GloriousThemes Starter Sites.', 'glorious-sites-installer' );
			}
		}
	...
	public function add_help_tabs() {
		...
		$screen->add_help_tab(
			array(
				'id'      => 'gloriousthemes_demo_importer_support_tab',
				'title'   => __( 'Help &amp; Support', 'glorious-sites-installer' ),
				'content' =>
					'<h2>' . __( 'Help &amp; Support', 'glorious-sites-installer' ) . '</h2>' .
				'<p>' . sprintf(
					__( 'Should you need help understanding, using, or extending GloriousThemes Starter Sites, <a href="%s">please read our documentation</a>. You will find all kinds of resources including snippets, tutorials and much more.', 'glorious-sites-installer' ),
					'https://gloriousthemes.com/docs//glorious-sites-installer/'
				) . '</p>' .
				'<p>' . sprintf(
					__( 'For further assistance with GloriousThemes Starter Sites core you can use the <a href="%1$s">community forum</a>. If you need help with premium themes sold by GloriousThemes, please <a href="%2$s">use our free support forum</a>.', 'glorious-sites-installer' ),
					'https://wordpress.org/support/plugin/glorious-sites-installer',
					'https://gloriousthemes.com/support-forum/'
				) . '</p>' .
					'<p><a href="' . 'https://wordpress.org/support/plugin/glorious-sites-installer' . '" class="button button-primary">' . __( 'Community forum', 'glorious-sites-installer' ) . '</a> <a href="' . 'https://gloriousthemes.com/support-forum/' . '" class="button">' . __( 'GloriousThemes Support', 'glorious-sites-installer' ) . '</a></p>',
			)
		);

		$screen->add_help_tab(
			array(
				'id'      => 'gloriousthemes_demo_importer_bugs_tab',
				'title'   => __( 'Found a bug?', 'glorious-sites-installer' ),
				'content' =>
					'<h2>' . __( 'Found a bug?', 'glorious-sites-installer' ) . '</h2>' .
					'<p>' . sprintf( __( 'If you find a bug within GloriousThemes Starter Sites you can create a ticket via <a href="%1$s">Github issues</a>. Ensure you read the <a href="%2$s">contribution guide</a> prior to submitting your report. To help us solve your issue, please be as descriptive as possible.', 'glorious-sites-installer' ), 'https://github.com/gloriousthemes//glorious-sites-installer/issues?state=open', 'https://github.com/gloriousthemes//glorious-sites-installer/blob/master/.github/CONTRIBUTING.md' ) . '</p>' .
					'<p><a href="' . 'https://github.com/gloriousthemes//glorious-sites-installer/issues?state=open' . '" class="button button-primary">' . __( 'Report a bug', 'glorious-sites-installer' ) . '</a></p>',

			)
		);

		$screen->set_help_sidebar(
			'<p><strong>' . __( 'For more information:', 'glorious-sites-installer' ) . '</strong></p>' .
			'<p><a href="' . 'https://gloriousthemes.com/demo-importer/' . '" target="_blank">' . __( 'About Demo Importer', 'glorious-sites-installer' ) . '</a></p>' .
			'<p><a href="' . 'https://wordpress.org/plugins//glorious-sites-installer/' . '" target="_blank">' . __( 'WordPress.org project', 'glorious-sites-installer' ) . '</a></p>' .
			'<p><a href="' . 'https://github.com/gloriousthemes/glorious-sites-installer' . '" target="_blank">' . __( 'Github project', 'glorious-sites-installer' ) . '</a></p>' .
			'<p><a href="' . 'https://gloriousthemes.com/wordpress-themes/' . '" target="_blank">' . __( 'Official themes', 'glorious-sites-installer' ) . '</a></p>' .
			'<p><a href="' . 'https://gloriousthemes.com/plugins/' . '" target="_blank">' . __( 'Official plugins', 'glorious-sites-installer' ) . '</a></p>'
		);
	}

	/**
	 * Disable the WooCommerce Setup Wizard on `GloriousThemes Starter Sites` page only.
	 */
	...
	/**
	 * Check for Ayush Premium theme plan.
	 *
	 * @return bool
	 */
	public function ayush_is_premium_theme_plan() {

		if ( is_plugin_active( 'ayush-pro/ayush-pro.php' ) && is_plugin_active( 'companion-elementor/companion-elementor.php') ) {
			return true;
		}
	...
	/**
	 * Check for GloriousThemes All Themes Plan
	 *
	 * @return bool
	 */
	public function gloriousthemes_is_all_themes_plan() {
	...
	public function ajax_query_demos( $return = true ) {
		...
		$demo_activated_id     = get_option( 'gloriousthemes_demo_importer_activated_id' );
		$available_packages    = $this->demo_packages;

		// Condition for Ayush Pro.
		$ayush_pro_plugin_version        = is_plugin_active( 'ayush-pro/ayush-pro.php' ) ? ZAKRA_PRO_VERSION : false;
		 ...
		 */
		$prepared_demos = (array) apply_filters( 'gloriousthemes_demo_importer_pre_prepare_demos_for_js', array(), $available_packages, $demo_activated_id );
		...
		if ( isset( $available_packages->demos ) ) {
			foreach ( $available_packages->demos as $package_slug => $package_data ) {
				...
				//Github link removed as per WP Requirements
				$screenshot_url = "https://raw.githubusercontent.com/GloriousThemes/gloriousthemes-demo-content/master/resources/{$available_packages->slug}/{$package_slug}/screenshot.jpg";
				/*
				* Glorious Themes OWN CDN Storage added
				* URL - https://gloriousproducts.b-cdn.net/ (Bunny CDN link)
				* 
				...
				* 
				...
				//https://gloriousproducts.b-cdn.net/GloriousThemes/gloriousthemes-demo-content/resources/ayush/ayush-pro/screenshot.jpg
				//$screenshot_url = "https://gloriousproducts.b-cdn.net/GloriousThemes/gloriousthemes-demo-content/resources/{$available_packages->slug}/{$package_slug}/screenshot.jpg";

				$screenshot_url = plugin_dir_url ( __DIR__ ) . "gloriousthemes-demo-content/resources/{$available_packages->slug}/{$package_slug}/screenshot.jpg"; 
				*/
				...
				$ayush_pro_required_version           = false;
				...
				if ( isset( $package_data->minimum_version ) && is_object( $package_data->minimum_version ) ) {
					foreach ( $package_data->minimum_version as $theme => $minimum_version ) {
						if ( 'ayush' === $current_template ) {
							if (
								version_compare( $minimum_version, $current_theme_version, '>' ) ||
								( 'ayush-pro' === $theme && version_compare( $minimum_version, $ayush_pro_plugin_version, '>' ) ) ||
								...
								if ( 'ayush-pro' === $theme ) {
									$ayush_pro_required_version = $minimum_version;
								}
				...
				if ( 'ayush' === $current_template ) {
					if ( $required_version ) {
						$required_message = sprintf( esc_html__( 'This demo requires %1$s version of %2$s theme to get imported', 'glorious-sites-installer' ), $required_version, $current_theme_name );
					}

					if ( $ayush_pro_required_version ) {
						if ( $required_version && $companion_elementor_required_version ) {
							$required_message = sprintf(
								esc_html__( 'This demo requires %1$s version of %2$s theme and %3$s version of %4$s as well as %5$s version of %6$s plugins to get imported', 'glorious-sites-installer' ),
								$required_version,
								$current_theme_name,
								$ayush_pro_required_version,
								esc_html__( 'Ayush Pro', 'glorious-sites-installer' ),
								$companion_elementor_required_version,
								esc_html__( 'Companion Elementor', 'glorious-sites-installer' )
							);
						} elseif ( $required_version ) {
							$required_message = sprintf(
								esc_html__( 'This demo requires %1$s version of %2$s theme and %3$s version of %4$s plugin to get imported', 'glorious-sites-installer' ),
								$required_version,
								$current_theme_name,
								$ayush_pro_required_version,
								esc_html__( 'Ayush Pro', 'glorious-sites-installer' )
							...
							$required_message = sprintf(
								esc_html__( 'This demo requires %1$s version of %2$s plugin to get imported', 'glorious-sites-installer' ),
								$ayush_pro_required_version,
								esc_html__( 'Ayush Pro', 'glorious-sites-installer' )
							);
						}
					...
					if ( $companion_elementor_required_version ) {
						if ( $required_version && $ayush_pro_required_version ) {
							$required_message = sprintf(
								esc_html__( 'This demo requires %1$s version of %2$s theme and %3$s version of %4$s as well as %5$s version of %6$s plugins to get imported', 'glorious-sites-installer' ),
								$required_version,
								$current_theme_name,
					...
					if ( ! $required_version && ( $ayush_pro_required_version && $companion_elementor_required_version ) ) {
						$required_message = sprintf(
							esc_html__( 'This demo requires %1$s version of %2$s as well as %3$s version of %4$s plugins to get imported', 'glorious-sites-installer' ),
							$ayush_pro_required_version,
							esc_html__( 'Ayush Pro', 'glorious-sites-installer' ),
							$companion_elementor_required_version,
							esc_html__( 'Companion Elementor', 'glorious-sites-installer' )
					...
					if ( $required_version ) {
						$required_message = sprintf(
							esc_html__( 'This demo requires %1$s version of %2$s theme to get imported', 'glorious-sites-installer' ),
							$required_version,
							$current_theme_name
						);
					}
				...
				$prepared_demos[ $package_slug ] = array(
					...
					'theme'             => $is_pro_theme_demo ? sprintf( esc_html__( '%s Pro', 'glorious-sites-installer' ), $available_packages->name ) : $available_packages->name,
					'isPro'             => $is_pro_theme_demo ? false : isset( $package_data->isPro ),
					'isPremium'         => $this->ayush_is_premium_theme_plan() ? false : isset( $package_data->isPremium ),
					'isAllThemePlan'    => $this->gloriousthemes_is_all_themes_plan() ? false : isset( $package_data->isAllThemePlan ),
					'active'            => $package_slug === $demo_activated_id,
					'author'            => isset( $package_data->author ) ? $package_data->author : __( 'GloriousThemes', 'glorious-sites-installer' ),
				...
				unset( $ayush_pro_required_version );
		 ...
		 */
		$prepared_demos = apply_filters( 'gloriousthemes_demo_importer_prepare_demos_for_js', $prepared_demos );
	...
	public function ajax_import_demo() {
		...
		if ( empty( $_POST['slug'] ) ) {
			wp_send_json_error(
				array(
					...
					'errorMessage' => __( 'No demo specified.', 'glorious-sites-installer' ),
				)
			);
		}
		...
		if ( ! current_user_can( 'import' ) ) {
			$status['errorMessage'] = __( 'Sorry, you are not allowed to import content.', 'glorious-sites-installer' );
		...
		$result   = $upgrader->install( "https://github.com/gloriousthemes/gloriousthemes-demo-content/raw/master/packages/{$template}/{$slug}.zip" );
		...
		if ( is_wp_error( $result ) ) {
			...
			$status['errorMessage'] = __( 'Unable to connect to the filesystem. Please confirm your credentials.', 'glorious-sites-installer' );
		...
		do_action( 'gloriousthemes_ajax_before_demo_import' );
		...
		if ( ! empty( $demo_data ) ) {
			...
			update_option( 'gloriousthemes_demo_importer_activated_id', $slug );
			do_action( 'gloriousthemes_ajax_demo_imported', $slug, $demo_data );
		}
	...
	public function ajax_footer_text_rated() {
		...
		update_option( 'gloriousthemes_demo_importer_admin_footer_text_rated', 1 );
		wp_die();
	}
	...
	public function import_dummy_xml( $demo_id, $demo_data, $status ) {
		...
		do_action( 'gloriousthemes_ajax_before_dummy_xml_import', $demo_data, $demo_id );
		...
		if ( is_file( $import_file ) ) {
			...
			do_action( 'gloriousthemes_ajax_dummy_xml_imported', $demo_data, $demo_id );

			flush_rewrite_rules();
		} else {
			$status['errorMessage'] = __( 'The XML file dummy content is missing.', 'glorious-sites-installer' );
			wp_send_json_error( $status );
		}
	...
	public function import_customizer_data( $demo_id, $demo_data, $status ) {
		...
		if ( is_file( $import_file ) ) {
			...
			$status['errorMessage'] = __( 'The DAT file customizer data is missing.', 'glorious-sites-installer' );
			wp_send_json_error( $status );
		}
	...
	public function import_widget_settings( $demo_id, $demo_data, $status ) {
		...
		if ( is_file( $import_file ) ) {
			...
			$status['errorMessage'] = __( 'The WIE file widget content is missing.', 'glorious-sites-installer' );
			wp_send_json_error( $status );
		}
				...
				if ( ! empty( $menu_items ) ) {
					foreach ( $menu_items as $menu_item ) {
						if ( isset( $menu_item->url ) && isset( $menu_item->db_id ) && 'custom' == $menu_item->type ) {
							...
							if ( isset( $menu_parts['path'] ) && isset( $menu_parts['host'] ) && apply_filters( 'gloriousthemes_demo_importer_nav_menu_item_url_hosts', in_array( $menu_parts['host'], array( 'demo.gloriousthemes.com', 'ayushdemos.com' ) ) ) ) {
	...
	public function refresh_demo_lists() {
		...
		if ( isset( $_GET['refresh-demo-packages'] ) && isset( $_GET['_refresh_demo_packages_nonce'] ) ) {
			if ( ! wp_verify_nonce( $_GET['_refresh_demo_packages_nonce'], 'refresh_demo_packages' ) ) {
				wp_die( __( 'Action failed. Please refresh the page and retry.', 'glorious-sites-installer' ) );
			}

			$demo_packages = get_transient( 'gloriousthemes_demo_importer_packages' );

			if ( $demo_packages ) {
				delete_transient( 'gloriousthemes_demo_importer_packages' );
			}
