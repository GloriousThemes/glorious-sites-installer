...
class TG_Demo_Importer_Status {
	...
	public static function get_write_permission() {
		...
		if ( ! $error && is_writable( $tg_demo_pack_uploads_path ) ) {
			$output = __( 'All Fine', 'glorious-sites-installer' );
		} else {
			$output = __( 'There are some write permission errors on your site.', 'glorious-sites-installer' );
		}
	...
	public static function get_demo_server_connection_status() {
		...
		if ( is_wp_error( $package_file_server ) || 200 !== (int) $http_response_code ) {
			$output = __( 'There is a connection issue of your site to our demo pack services.', 'glorious-sites-installer' );
		} else {
			$output = __( 'Connected', 'glorious-sites-installer' );
		}
