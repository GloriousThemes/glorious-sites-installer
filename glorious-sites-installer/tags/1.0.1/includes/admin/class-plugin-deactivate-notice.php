...
class TG_Demo_Importer_Plugin_Deactivate_Notice {
	...
	public function deactivate_notice_markup() {
		$demo_imported            = get_option( 'gloriousthemes_demo_importer_activated_id' );
		...
		<div class="notice notice-success tg-demo-importer-notice plugin-deactivate-notice" style="position:relative;">
			...
			<div class="links">
				<a href="<?php echo esc_url( wp_nonce_url( add_query_arg( 'deactivate-glorious-sites-installer-plugin', 'true' ), 'deactivate_gloriousthemes_demo_importer_plugin', '_deactivate_gloriousthemes_demo_importer_plugin_nonce' ) ); ?>"
				   class="btn button-primary"
				>
					<span class="dashicons dashicons-thumbs-up"></span>
					<span><?php esc_html_e( 'Deactivate', 'glorious-sites-installer' ); ?></span>
				</a>
			</div> <!-- /.links -->
	...
	/**
	 * Deactivates the GloriousThemes Starter Sites plugin.
	 */
	public function deactivate_plugin() {
		// Deactivate the plugin.
		if ( isset( $_GET['deactivate-glorious-sites-installer-plugin'] ) && isset( $_GET['_deactivate_gloriousthemes_demo_importer_plugin_nonce'] ) ) {
			if ( ! wp_verify_nonce( $_GET['_deactivate_gloriousthemes_demo_importer_plugin_nonce'], 'deactivate_gloriousthemes_demo_importer_plugin' ) ) {
				wp_die( __( 'Action failed. Please refresh the page and retry.', 'glorious-sites-installer' ) );
			}

			// Get the plugin.
			$plugin = 'glorious-sites-installer/glorious-sites-installer.php';
