(function($, wp) {
    ...
    wp.updates.importDemo = function(args) {
        ...
        if ($message.html() !== __('Importing...', 'glorious-sites-installer')) {
            $message.data('originaltext', $message.html());
        }

        $message
            .text(__('Importing...', 'glorious-sites-installer'))
            .attr(
                'aria-label',
                sprintf(
                    /* translators: %s: Demo name. */
                    _x('Importing %s...', 'demo', 'glorious-sites-installer'),
                    $message.data('name')
                )
            );
        wp.a11y.speak(__('Importing... please wait.', 'glorious-sites-installer'), 'polite');
    ...
    wp.updates.importDemoSuccess = function(response) {
        ...
        $message = $card.find('.button-primary')
            ...
            .attr(
                ...
                sprintf(
                    /* translators: %s: Demo name. */
                    _x('%s imported!', 'demo', 'glorious-sites-installer'),
                    response.demoName
                )
            )
            .text(__('Imported!', 'glorious-sites-installer'));

        wp.a11y.speak(__('Import completed successfully.', 'glorious-sites-installer'), 'polite');
            ...
            if (response.previewUrl) {
                ...
                $message
                    ...
                    .attr(
                        ...
                        sprintf(
                            /* translators: %s: Demo name. */
                            _x('Live Preview %s', 'demo', 'glorious-sites-installer'),
                            response.demoName
                        )
                    )
                    .text(__('Live Preview', 'glorious-sites-installer'));
            }
        }, 1000);
    };
    ...
    wp.updates.importDemoError = function(response) {
        var $card, $button,
            errorMessage = sprintf(
                ...
                __('Import failed: %s', 'glorious-sites-installer'),
        ...
        $button
            ...
            .attr(
                ...
                sprintf(
                    /* translators: %s: Demo name. */
                    _x('%s import failed', 'demo', 'glorious-sites-installer'),
                    $button.data('name')
                )
            )
            .text(__('Import Failed!', 'glorious-sites-installer'));
    ...
    wp.updates.bulkInstallPlugin = function(args) {
        ...
        if ($document.find('body').hasClass('full-overlay-active')) {
            ...
            message = sprintf(
                ...
                _x('Installing %s...', 'plugin', 'glorious-sites-installer'),
                $pluginRow.data('name')
            );
            $pluginRow.find('.plugin-status span')
                ...
                .text(__('Installing...', 'glorious-sites-installer'));
        } else {
        ...
        if ($message.html() !== __('Installing...', 'glorious-sites-installer')) {
            $message.data('originaltext', $message.html());
        }
        ...
        wp.a11y.speak(__('Installing... please wait.', 'glorious-sites-installer'), 'polite');
    ...
    wp.updates.bulkInstallPluginSuccess = function(response) {
        ...
        $updateMessage
            ...
            .attr(
                ...
                sprintf(
                    /* translators: %s: Plugin name. */
                    _x('%s installed!', 'plugin', 'glorious-sites-installer'),
                    response.pluginName
                )
            )
            .text(_x('Installed!', 'plugin', 'glorious-sites-installer'));

        wp.a11y.speak(__('Installation completed successfully.', 'glorious-sites-installer'), 'polite');
    ...
    wp.updates.bulkInstallPluginError = function(response) {
        ...
        errorMessage = sprintf(
            ...
            __('Installation failed: %s', 'glorious-sites-installer'),
            response.errorMessage
        );
        ...
        $updateMessage
            ...
            .attr(
                ...
                sprintf(
                    /* translators: %s: Plugin name. */
                    _x('%s installation failed', 'plugin', 'glorious-sites-installer'),
                    $pluginRow.data('name')
                )
            )
            .text(__('Installation Failed!', 'glorious-sites-installer'));
    ...
    wp.updates.isValidResponse = function(response, action) {
        var error = __('Something went wrong.', 'glorious-sites-installer'),
            errorMessage;
        ...
        if (_.isString(response) && '-1' === response) {
            error = __('An error has occurred. Please reload the page and try again.', 'glorious-sites-installer');
        } else if (_.isString(response)) {
            error = response;
        } else if ('undefined' !== typeof response.readyState && 0 === response.readyState) {
            error = __('Connection lost or the server is busy. Please try again later.', 'glorious-sites-installer');
        } else if (_.isString(response.statusText)) {
            error = response.statusText + ' ' + '<a href="https://docs.gloriousthemes.com/knowledgebase/demo-import-process-failed/" target="_blank">' + __('Try this solution!', 'glorious-sites-installer') + '</a>';
        }

        switch (action) {
            case 'import':
                errorMessage = __('Import failed: %s', 'glorious-sites-installer');
                break;

            case 'install':
                errorMessage = __('Installation failed: %s', 'glorious-sites-installer');
                break;
        }
